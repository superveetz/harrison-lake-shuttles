...
class CheckoutForm extends React.Component<CheckoutFormProps & CheckoutFormReduxProps, {}> {
  ...
  private processStripeCharge(token: any, formikBag: FormikProps<CheckoutFormValues>): Promise<any> {
    return API.post("apihlsreststripe1", "/process-transaction", {
      body: {
        stripeToken: token,
        charge: {
          currency: "CAD",
  ...
  initReviewOrderData = () => {
    ...
    // apply promo code
    if (this.state.promoCodeApplied) {
      const amountSaved = this.return2SubTotal * this.promoCodeDiscountPerc;
      this.promoCodeAmountSaved += amountSaved;
      this.return2SubTotal -= amountSaved;
    }
  ...
  onPromoCodeFormSubmit(values: PromoCodeFormValues) {
    ...
    if (values.promoCode.toLowerCase() === "hotelsharrison") {
      this.setState({
              ...
              return (
                <form onSubmit={formikBag.handleSubmit}>
                  <div className="row w-100">
                    ...
                    <div className="col-3">
                      <Button size="xs" theme="secondary" style={{ marginTop: "20px" }} btnType="submit">
                        Apply
                    </Button>
                    </div>
                  </div>
  ...
  resendOrderConfirmationEmail(): void {
    ...
    API.post("apihlsreststripe1", "/resend-order-confirmation-email", {
      body: {
        charge: {
    ...
    // fetch("http://localhost:3000/resend-order-confirmation-email", {
    //   headers: {
    //     Accept: "application/json",
    //     "Content-Type": "application/json",
    //   },
